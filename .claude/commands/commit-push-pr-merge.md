...
description: 変更をコミット、プッシュし、PRを作成してマージ
---
...
5. **プルリクエストをマージ**
   - CIチェックが完了するまで待機（必要に応じて）
   - `gh pr merge --auto --squash`または`gh pr merge --squash`を使用
   - スカッシュマージを推奨（履歴をクリーンに保つため）
   - マージ後、feature/security-dashboardブランチに戻る
