# ITSM-Sec Nexus - 開発環境・本番環境 分離運用ガイド

## 📋 目次

1. [環境構成の概要](#環境構成の概要)
2. [開発ワークフロー](#開発ワークフロー)
3. [データベース管理](#データベース管理)
4. [設定ファイル管理](#設定ファイル管理)
5. [デプロイプロセス](#デプロイプロセス)
6. [ログ管理とモニタリング](#ログ管理とモニタリング)
7. [バックアップ戦略](#バックアップ戦略)
8. [チーム開発](#チーム開発)
9. [トラブルシューティング](#トラブルシューティング)

---

## 環境構成の概要

### 🌍 環境マトリックス

| 環境 | URL | ポート | プロトコル | NODE_ENV | データベース | 用途 |
|------|-----|--------|-----------|----------|------------|------|
| **開発環境** | `https://192.168.0.187:5443` | 5443 | HTTPS | development | itsm_nexus.db | 機能開発・デバッグ |
| **本番環境** | `https://192.168.0.187:6443` | 6443 | HTTPS | production | itsm_nexus.db | 実運用 |

### 🎯 環境分離の目的

✅ **開発と本番の完全分離**
- 本番環境を壊すリスクゼロ
- 自由な実験が可能

✅ **視覚的な環境識別**
- 開発環境: 緑色バッジ `🔧 開発環境`
- 本番環境: 赤色バッジ `🔒 本番環境`
- ブラウザタブ: `[開発]` / `[本番]`

✅ **独立したサービス管理**
- 片方だけを起動/停止可能
- 異なるリソース制限
- 別々のログファイル

---

## 開発ワークフロー

### 🔄 基本フロー

```
┌─────────────┐
│  機能開発    │  開発環境で新機能を実装
│  (Dev)      │  https://192.168.0.187:5443
└──────┬──────┘
       │
       ▼
┌─────────────┐
│  テスト      │  開発環境で動作確認
│  (Dev)      │  ・機能テスト
└──────┬──────┘  ・UIテスト
       │         ・統合テスト
       ▼
┌─────────────┐
│ コミット     │  Gitにコミット
│ (Git)       │  git commit -m "..."
└──────┬──────┘
       │
       ▼
┌─────────────┐
│  デプロイ    │  本番環境に反映
│  (Prod)     │  本番サービスを再起動
└──────┬──────┘
       │
       ▼
┌─────────────┐
│ 検証・監視   │  本番環境で動作確認
│  (Prod)     │  ログ監視
└─────────────┘
```

### 📝 日常的な開発フロー

#### **ステップ1: 開発環境で作業**

```bash
# 開発環境のみ起動
sudo ./manage-env.sh prod stop
sudo ./manage-env.sh dev start

# ログをリアルタイム監視
./manage-env.sh dev logs
```

ブラウザで開く：
```
https://192.168.0.187:5443
```

#### **ステップ2: コード変更**

```bash
# フロントエンド修正
nano frontend/app.js
nano frontend/style.css

# バックエンド修正
nano backend/routes/incidents.js
nano backend/middleware/auth.js
```

#### **ステップ3: 開発環境で即座にテスト**

```bash
# 開発環境を再起動
sudo ./manage-env.sh dev restart

# ブラウザでテスト
# https://192.168.0.187:5443
```

#### **ステップ4: テストが完了したらコミット**

```bash
git add .
git commit -m "feat: 新しい機能を追加"
```

#### **ステップ5: 本番環境にデプロイ**

```bash
# 本番環境を再起動して変更を反映
sudo ./manage-env.sh prod restart

# 本番環境で動作確認
# https://192.168.0.187:6443
```

---

## データベース管理

### 🗄️ データベース戦略

#### **現在の構成**

- **共通データベース**: `backend/itsm_nexus.db`
- 両環境が同じDBを参照

#### **推奨構成（将来）**

```
backend/
  ├── itsm_nexus_dev.db      # 開発環境専用
  └── itsm_nexus_prod.db     # 本番環境専用
```

### 📋 データベース分離の実装

#### **1. 環境変数ファイルを更新**

`.env.development`:
```bash
DATABASE_PATH=./backend/itsm_nexus_dev.db
```

`.env.production`:
```bash
DATABASE_PATH=./backend/itsm_nexus_prod.db
```

#### **2. 開発用DBを作成**

```bash
# 本番DBをコピーして開発DBを作成
cp backend/itsm_nexus.db backend/itsm_nexus_dev.db

# または、マイグレーションから新規作成
cd backend
node -e "require('./db').initDb()"
```

#### **3. データベースバックアップスクリプト**

```bash
#!/bin/bash
# backup-db.sh

DATE=$(date +%Y%m%d_%H%M%S)

# 本番DBをバックアップ
cp backend/itsm_nexus.db "backups/itsm_nexus_prod_${DATE}.db"

# 開発DBをバックアップ（週次）
if [ $(date +%u) -eq 1 ]; then
    cp backend/itsm_nexus_dev.db "backups/itsm_nexus_dev_${DATE}.db"
fi

# 30日以上古いバックアップを削除
find backups/ -name "*.db" -mtime +30 -delete

echo "✅ バックアップ完了: ${DATE}"
```

### 🔄 本番データを開発環境に同期

```bash
# 本番DBのスナップショットを取得
cp backend/itsm_nexus.db "backups/itsm_nexus_$(date +%Y%m%d).db"

# 開発DBに上書き（注意：開発データは失われます）
cp backend/itsm_nexus.db backend/itsm_nexus_dev.db

# 開発環境を再起動
sudo ./manage-env.sh dev restart
```

---

## 設定ファイル管理

### 📁 環境別設定ファイル

| ファイル | 説明 | バージョン管理 |
|---------|------|--------------|
| `.env.development` | 開発環境設定 | ✅ Gitにコミット |
| `.env.production` | 本番環境設定 | ⚠️ .gitignoreに追加 |
| `.env.example` | 設定テンプレート | ✅ Gitにコミット |

### 🔒 機密情報の管理

#### **推奨事項**

1. **.gitignoreに追加**

```bash
# .gitignore
.env.production
*.db
*.db-wal
*.db-shm
backend-*.log
ssl/*.key
```

2. **本番環境の機密情報**

```bash
# .env.production は手動で管理
# 以下の情報は絶対にGitにコミットしない：
- JWT_SECRET（本番用）
- SMTP_PASSWORD
- M365_CLIENT_SECRET
- SSL秘密鍵
```

3. **環境変数の検証**

```bash
# 起動時に必須環境変数をチェック
node -e "
if (!process.env.JWT_SECRET) {
  console.error('❌ JWT_SECRET が設定されていません');
  process.exit(1);
}
console.log('✅ 環境変数チェック完了');
"
```

---

## デプロイプロセス

### 🚀 開発→本番へのデプロイ手順

#### **パターン1: 小規模な変更（推奨）**

```bash
# 1. 開発環境でテスト済みであることを確認
./manage-env.sh dev status

# 2. Gitにコミット
git add frontend/app.js backend/routes/incidents.js
git commit -m "fix: インシデント管理の表示バグを修正"

# 3. 本番環境を再起動
sudo ./manage-env.sh prod restart

# 4. 本番環境で動作確認
# https://192.168.0.187:6443

# 5. ログを監視
./manage-env.sh prod logs
```

#### **パターン2: 大規模な変更**

```bash
# 1. 本番DBをバックアップ
cp backend/itsm_nexus.db "backups/itsm_nexus_$(date +%Y%m%d_%H%M%S).db"

# 2. 本番環境を停止
sudo ./manage-env.sh prod stop

# 3. DBマイグレーションを実行（必要な場合）
cd backend
npm run migrate

# 4. 本番環境を起動
sudo ./manage-env.sh prod start

# 5. ヘルスチェック
curl -k https://192.168.0.187:6443/health

# 6. 監視
./manage-env.sh prod logs
```

#### **パターン3: ロールバック**

```bash
# 1. 本番環境を停止
sudo ./manage-env.sh prod stop

# 2. 以前のコミットに戻る
git log --oneline -10
git checkout <commit-hash>

# 3. DBを復元（必要な場合）
cp backups/itsm_nexus_20260116_100000.db backend/itsm_nexus.db

# 4. 本番環境を起動
sudo ./manage-env.sh prod start
```

---

## ログ管理とモニタリング

### 📊 ログファイル

| 環境 | ログファイル | 場所 |
|------|------------|------|
| 開発環境 | `backend-dev.log` | プロジェクトルート |
| 本番環境 | `backend-prod.log` | プロジェクトルート |

### 🔍 ログ監視コマンド

```bash
# 開発環境のログをリアルタイム表示
tail -f backend-dev.log

# 本番環境のログをリアルタイム表示
tail -f backend-prod.log

# エラーのみ抽出
grep -i "error\|failed\|exception" backend-prod.log

# 過去1時間のログ
journalctl -u itsm-sec-nexus-prod --since "1 hour ago"
```

### 📈 監視すべき項目

#### **開発環境**

- JavaScriptエラー
- API応答時間
- データベースクエリ性能

#### **本番環境**

- サービス稼働率
- エラー発生率
- API応答時間
- メモリ使用量
- CPU使用率
- 認証失敗回数

### 🔔 アラート設定（推奨）

```bash
# 本番環境のエラーを監視するスクリプト
# watch-prod-errors.sh

#!/bin/bash
tail -f backend-prod.log | while read line; do
    if echo "$line" | grep -qi "error\|failed\|exception"; then
        # 通知を送信（例：Slack、メール）
        echo "🚨 本番環境でエラー検出: $line"
        # curl -X POST <webhook-url> -d "..."
    fi
done
```

---

## バックアップ戦略

### 💾 バックアップ計画

| 対象 | 頻度 | 保持期間 | 方法 |
|------|------|---------|------|
| **本番DB** | 毎日 | 30日 | 自動バックアップ |
| **開発DB** | 週次 | 7日 | 自動バックアップ |
| **設定ファイル** | 変更時 | Git履歴 | Gitコミット |
| **SSL証明書** | 更新時 | 永久 | 手動バックアップ |

### 📝 自動バックアップスクリプト

```bash
#!/bin/bash
# daily-backup.sh

BACKUP_DIR="/mnt/LinuxHDD/ITSM-System/backups"
DATE=$(date +%Y%m%d_%H%M%S)

mkdir -p "$BACKUP_DIR"

# 本番DBをバックアップ
cp backend/itsm_nexus.db "${BACKUP_DIR}/itsm_nexus_prod_${DATE}.db"

# 設定ファイルをバックアップ
cp .env.production "${BACKUP_DIR}/.env.production_${DATE}.bak"

# 圧縮
tar -czf "${BACKUP_DIR}/backup_${DATE}.tar.gz" \
    backend/itsm_nexus.db \
    .env.production \
    ssl/

# 30日以上古いバックアップを削除
find "$BACKUP_DIR" -name "*.db" -mtime +30 -delete
find "$BACKUP_DIR" -name "*.tar.gz" -mtime +30 -delete

echo "✅ バックアップ完了: ${DATE}"
```

### ⏰ cronで自動実行

```bash
# crontabに追加
crontab -e

# 毎日午前3時にバックアップ
0 3 * * * /mnt/LinuxHDD/ITSM-System/daily-backup.sh >> /mnt/LinuxHDD/ITSM-System/backup.log 2>&1
```

---

## チーム開発

### 👥 役割分担

| 役割 | アクセス権限 | 主な作業 |
|------|------------|---------|
| **開発者** | 開発環境 | 機能実装、バグ修正 |
| **テスター** | 開発環境 | QA、テストケース実行 |
| **管理者** | 両環境 | デプロイ、設定変更 |
| **運用担当** | 本番環境（読み取り） | 監視、ログ確認 |

### 🔀 ブランチ戦略

```
main (本番環境)
  │
  ├── develop (開発環境)
  │     │
  │     ├── feature/incident-filter
  │     ├── feature/sla-dashboard
  │     └── bugfix/authentication
  │
  └── hotfix/security-patch (緊急修正)
```

#### **ワークフロー**

```bash
# 1. 新機能の開発
git checkout -b feature/new-feature develop
# 開発環境で作業
# https://192.168.0.187:5443

# 2. 開発ブランチにマージ
git checkout develop
git merge feature/new-feature

# 3. 開発環境でテスト
sudo ./manage-env.sh dev restart

# 4. 本番にマージ
git checkout main
git merge develop

# 5. 本番環境にデプロイ
sudo ./manage-env.sh prod restart
```

### 📋 コードレビュープロセス

1. **プルリクエスト作成**
2. **開発環境で動作確認**
3. **コードレビュー**
4. **マージ**
5. **本番デプロイ**

---

## トラブルシューティング

### 🔧 よくある問題と解決策

#### **問題1: 開発環境と本番環境で挙動が違う**

**原因**: 環境変数の違い

**解決**:
```bash
# 環境変数の差分を確認
diff .env.development .env.production

# 特に以下をチェック：
- NODE_ENV
- LOG_LEVEL
- RATE_LIMIT設定
- CORS_ORIGIN
```

#### **問題2: 本番環境でエラーが頻発**

**解決**:
```bash
# エラーログを確認
tail -100 backend-prod.log | grep -i error

# リソース使用状況を確認
./manage-env.sh prod status

# 必要に応じてロールバック
git checkout <previous-commit>
sudo ./manage-env.sh prod restart
```

#### **問題3: データベースが壊れた**

**解決**:
```bash
# バックアップから復元
cp backups/itsm_nexus_prod_<DATE>.db backend/itsm_nexus.db

# サービスを再起動
sudo ./manage-env.sh prod restart
```

---

## セキュリティベストプラクティス

### 🔐 開発環境

- ✅ サンプルデータの使用OK
- ✅ デバッグログ有効
- ⚠️ 本番の機密データは使用しない

### 🔒 本番環境

- ❌ サンプルデータ無効
- ❌ デバッグログ無効
- ✅ 強力なJWT_SECRET
- ✅ レート制限を厳しく設定
- ✅ 定期的なバックアップ
- ✅ ログ監視

---

## クイックリファレンス

### 🚀 よく使うコマンド

```bash
# 開発作業開始
sudo ./manage-env.sh dev start
./manage-env.sh dev logs

# 開発環境でテスト
sudo ./manage-env.sh dev restart

# 本番にデプロイ
sudo ./manage-env.sh prod restart

# 両方の状態を確認
./manage-env.sh dev status
./manage-env.sh prod status

# ログを確認
tail -f backend-dev.log
tail -f backend-prod.log
```

---

## 📚 関連ドキュメント

- **環境セットアップ**: `ENVIRONMENT_SETUP.md`
- **Systemdサービス管理**: `SYSTEMD_SERVICE.md`
- **データベース設計**: `DATABASE_DESIGN.md`
- **API仕様**: `openapi.yaml`

---

**ITSM-Sec Nexus Team**
