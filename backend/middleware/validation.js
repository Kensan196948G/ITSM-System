...
const incidentValidation = {
    create: [
        body('title')
            ...
            .isLength({ max: 500 }).withMessage('タイトルは500文字以内で入力してください'),
        body('priority')
            .notEmpty().withMessage('優先度は必須です')
            .isIn(['Critical', 'High', 'Medium', 'Low']).withMessage('優先度が無効です'),
        body('status')
            .optional()
        ...
        body('is_security_incident')
            .optional()
            .isBoolean().withMessage('セキュリティインシデントフラグはboolean値である必要があります')
    ],
    update: [
...
const changeValidation = {
    create: [
        ...
        body('asset_tag')
            .optional()
            .isLength({ max: 50 }).withMessage('資産タグは50文字以内で入力してください'),
        body('requester')
            .trim()
            .notEmpty().withMessage('申請者は必須です'),
        body('impact_level')
            .optional()
            .isIn(['Low', 'Medium', 'High']).withMessage('影響度が無効です'),
        body('is_security_change')
            .optional()
            .isBoolean().withMessage('セキュリティ変更フラグはboolean値である必要があります')
    ],
    update: [
        param('id').notEmpty().withMessage('RFC IDは必須です'),
        body('status')
            .optional()
            .isIn(['Pending', 'Approved', 'Rejected', 'Implemented']).withMessage('ステータスが無効です'),
        body('approver')
            .optional()
            .trim()
            .isLength({ max: 100 }).withMessage('承認者は100文字以内で入力してください')
    ]
};
...
const authValidation = {
    register: [
        body('username')
            ...
            .matches(/^[a-zA-Z0-9_-]+$/).withMessage('ユーザー名は英数字、ハイフン、アンダースコアのみ使用できます'),
        body('email')
            .trim()
        ...
        body('role')
            .optional()
            .isIn(['admin', 'manager', 'analyst', 'viewer']).withMessage('ロールが無効です'),
        body('full_name')
            .optional()
            .trim()
