...
exports.up = function (knex) {
  return knex.schema
    ...
    .createTable('csf_categories', (table) => {
      table.increments('id').primary();
      table.integer('function_id').unsigned().references('id').inTable('csf_functions').onDelete('CASCADE');
    ...
    .createTable('csf_controls', (table) => {
      table.increments('id').primary();
      table.integer('category_id').unsigned().references('id').inTable('csf_categories').onDelete('CASCADE');
      table.string('control_id', 30).notNullable().unique(); // GV.OC-01, ID.AM-01, etc.
      table.string('name', 200).notNullable();
      table.text('description');
      table.enu('status', ['not_started', 'in_progress', 'partial', 'compliant', 'non_compliant']).defaultTo('not_started');
    ...
    .createTable('csf_assessments', (table) => {
      table.increments('id').primary();
      table.integer('control_id').unsigned().references('id').inTable('csf_controls').onDelete('CASCADE');
      table.integer('assessed_by').unsigned().references('id').inTable('users');
      table.enu('previous_status', ['not_started', 'in_progress', 'partial', 'compliant', 'non_compliant']);
      table.enu('new_status', ['not_started', 'in_progress', 'partial', 'compliant', 'non_compliant']);
    ...
    .then(() => {
      return knex('csf_functions').insert([
        { code: 'GV', name: '統治', name_en: 'Govern', description: '組織のサイバーセキュリティリスク管理戦略、期待値、ポリシーの確立と監視', color: 'govern', icon: 'fa-balance-scale', sort_order: 1 },
        { code: 'ID', name: '識別', name_en: 'Identify', description: '組織の資産、リスク、脆弱性の識別と理解', color: 'identify', icon: 'fa-search', sort_order: 2 },
        { code: 'PR', name: '防御', name_en: 'Protect', description: '重要なサービスの提供を確保するための適切な保護措置の実装', color: 'protect', icon: 'fa-shield-alt', sort_order: 3 },
        { code: 'DE', name: '検知', name_en: 'Detect', description: 'サイバーセキュリティイベントの発生をタイムリーに検知するための活動', color: 'detect', icon: 'fa-eye', sort_order: 4 },
        { code: 'RS', name: '対応', name_en: 'Respond', description: '検知されたサイバーセキュリティインシデントへの対応', color: 'respond', icon: 'fa-bolt', sort_order: 5 },
        { code: 'RC', name: '復旧', name_en: 'Recover', description: 'サイバーセキュリティインシデントにより損なわれた機能やサービスの復旧', color: 'recover', icon: 'fa-sync-alt', sort_order: 6 }
      ]);
    });
};
