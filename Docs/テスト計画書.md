...
| カテゴリ | 現状 | 目標 | 期限 |
|---------|-----|------|------|
| **Statements** | 46.63% | 85% | Phase A-5 |
| **Branches** | 36.92% | 80% | Phase A-5 |
| **Functions** | 55.55% | 85% | Phase A-5 |
| **Lines** | 47.08% | 85% | Phase A-5 |

**中間目標（Phase A-4）**: Statements/Branches/Functions/Lines の各50%

### 4.3 未カバー領域

**優先度High（カバレッジ向上必要）**:
- backend/server.js（主要ルート分岐）
- backend/db.js（初期化/シード/エラーハンドリング）
- backend/middleware/userActivity.js（監査/集計系）
- backend/routes/auth/2fa.js（2FAフロー）
- backend/routes/auth/passwordReset.js（パスワードリセット）
- backend/routes/export.js / backend/services/exportService.js（出力処理）
...
# 未カバー箇所特定
node -e "const {createCoverageMap}=require('istanbul-lib-coverage'); const fs=require('fs'); const map=createCoverageMap(JSON.parse(fs.readFileSync('coverage/coverage-final.json','utf8'))); console.log(map.getCoverageSummary().toJSON());"
```
