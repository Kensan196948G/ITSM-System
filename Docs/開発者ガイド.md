...
- **ダッシュボード**: KPI可視化、リアルタイムウィジェット、チャート表示
...
- **通知センター**: 未読通知、通知履歴、通知設定
- **レポート管理**: スケジュールレポート、レポート生成、レポート履歴
- **統合設定**: Microsoft 365、ServiceNow、Webhook設定
                                    ▼
┌─────────────────────────────────────────────────────────────┐
│                   Express.js サーバー                         │
│  ┌──────────────────────────────────────────────────────┐  │
...
│  ┌──────────────────────────────────────────────────────┐  │
│  │ APIエンドポイント                                       │  │
│  │  /api/v1/auth/*           - 認証・2FA・パスワードリセット  │  │
│  │  /api/v1/dashboard/*      - KPI統計・ウィジェット         │  │
│  │  /api/v1/incidents/*      - インシデント管理             │  │
│  │  /api/v1/changes/*        - 変更管理                    │  │
│  │  /api/v1/problems/*       - 問題管理                    │  │
│  │  /api/v1/assets/*         - 構成管理                    │  │
│  │  /api/v1/sla/*            - SLA管理・アラート            │  │
│  │  /api/v1/security/*       - セキュリティ管理             │  │
│  │  /api/v1/vulnerabilities/* - 脆弱性管理                 │  │
│  │  /api/v1/audit-logs/*     - 監査ログ                    │  │
│  │  /api/v1/reports/*        - レポート生成                │  │
│  │  /api/v1/notifications/*  - 通知管理                    │  │
│  │  /api/v1/integrations/*   - 外部システム統合            │  │
│  │  /api/v1/webhooks/*       - Webhook管理                │  │
│  │  /api/v1/export/*         - データエクスポート           │  │
...
| **node-cron** | ^4.2.1 | スケジュール実行 |
| **nodemailer** | ^7.0.12 | メール送信 |
| **pdfkit** | ^0.17.2 | PDF生成 |
| **axios** | ^1.13.2 | HTTP クライアント |
| **qrcode** | ^1.5.4 | QRコード生成（2FA用） |
| **speakeasy** | ^2.0.0 | TOTP生成（2FA用） |
...
```bash
chmod +x scripts/startup/start-system.sh
...
#### 2026-01-07 (v2.0.0)
- **ダッシュボード & レポート**: 統合ダッシュボード、PDFレポート生成、スケジュールレポート機能追加
- **通知システム**: マルチチャネル通知（メール/Slack/Webhook）実装
- **統合機能**: Microsoft 365、ServiceNow統合実装
- **API拡張**: 全60以上のエンドポイント追加
  - `/api/v1/reports/*` - レポート生成API
  - `/api/v1/notifications/*` - 通知管理API
  - `/api/v1/integrations/*` - 統合API
  - `/api/v1/webhooks/*` - Webhook API
- **パフォーマンス最適化**: インメモリキャッシュ、Prometheusメトリクス追加
- **技術スタック追加**: node-cache, node-cron, nodemailer, pdfkit, axios, handlebars, prom-client
- **E2Eテスト**: Playwright E2Eテスト追加（ダッシュボード、レポート、統合、通知）
- **OpenAPI仕様**: Swagger UI統合（/api-docs）

#### 2026-01-03 (v1.5.0)
- SLA管理機能（違反アラート、レポート生成）の実装を反映
- 新規APIエンドポイント: `/api/v1/sla-reports/generate`
- フロントエンド: SLAレポートモーダル・PDF/Excel出力機能
