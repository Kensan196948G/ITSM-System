...
SLA管理画面から期間を指定してパフォーマンスレポートを生成できます：

1. サイドメニューから「SLA管理」を選択
2. 「レポート生成」ボタンをクリック
3. 期間を指定（任意）
4. 出力形式を選択（PDF/Excel/プレビュー）

### SLA違反アラート

SLA契約のステータスが違反状態になると、自動でメールアラートが送信されます：

- **トリガー条件**: ステータスがViolated/Breached、達成率が閾値以下
- **通知先**: `.env`の`SLA_ALERT_EMAIL`で設定
