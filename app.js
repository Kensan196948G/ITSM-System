...
async function renderDashboard(container) {
    try {
        ...
        csfItems.forEach(item => {
            ...
            headerDiv.appendChild(createEl('span', { textContent: item.label, style: 'font-weight: 600;' }));
            headerDiv.appendChild(createEl('span', { textContent: `${item.value}%`, style: 'font-weight: 700;' }));
...
async function renderIncidents(container) {
    try {
        ...
        const createBtn = createEl('button', { className: 'btn-primary', textContent: '新規インシデント作成' });
        ...
        incidents.forEach(inc => {
            ...
            row.appendChild(createEl('td', { textContent: new Date(inc.created_at).toLocaleString('ja-JP') }));
...
function showIncidentDetail(incident) {
    alert(`インシデント詳細: ${incident.ticket_id}\nタイトル: ${incident.title}\n\n詳細モーダル機能は次のフェーズで実装予定`);
}
...
async function renderChanges(container) {
    try {
        ...
        changes.forEach(change => {
            ...
            row.appendChild(createEl('td', { textContent: new Date(change.created_at).toLocaleString('ja-JP') }));
...
async function renderCMDB(container) {
    try {
        ...
        assets.forEach(asset => {
            ...
            row.appendChild(createEl('td', { textContent: new Date(asset.last_updated).toLocaleString('ja-JP') }));
...
async function renderSecurity(container) {
    ...
    const p = createEl('p', { textContent: 'NIST CSF 2.0の6つの機能（GOVERN, IDENTIFY, PROTECT, DETECT, RESPOND, RECOVER）に基づくセキュリティ管理を実施します。' });
